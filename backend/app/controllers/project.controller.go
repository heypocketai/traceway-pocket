...
func (p projectController) CreateProject(c *gin.Context) {
	...
	if !validFrameworks[request.Framework] {
		...
		c.JSON(http.StatusBadRequest, gin.H{"error": "Framework must be one of: gin, fiber, chi, fasthttp, stdlib, custom, react, svelte, vuejs, nextjs, nestjs, express, remix, cloudflare, opentelemetry, symfony"})
		return
	}
