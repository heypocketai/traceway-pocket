...
Logs and actions are sent to the backend inside the same `sessionRecordings` entry that carries the MP4 — they're stored under separate `logs` and `actions` arrays:
  ...
  "sessionRecordings": [
    {
      ...
      "logs": [
        {"type": "log", "timestamp": "...", "level": "info", "message": "user tapped pay"}
      ],
      "actions": [
        {"type": "navigation", "action": "push", "from": "/", "to": "/cart", "timestamp": "..."},
        {"type": "network", "method": "GET", "url": "...", "statusCode": 200, "durationMs": 86, "timestamp": "..."},
        {"type": "custom", "category": "cart", "name": "add_item", "data": {"sku": "SKU-1"}, "timestamp": "..."}
      ]
    }
  ]
}
...
A session recording entry is created for every captured exception that has any timeline data — even when `screenCapture: false` and there's no video.
